...
let updatedIndex = 0;
let titleRegex = /^\S[a-z0-9\s]{1,18}\S$/i;
let descriptionRegex = /^\S[a-z0-9\s]{3,98}\S$/i;
...
function addTask() {
    if (makeValidate(titleRegex, titleInput) && makeValidate(descriptionRegex, descriptionInput)) {
        let task = {
...
function displayTasks(index) {
    ...
    let theTask = `
        <div class="task" style="background-color: ${task.backgroundColor || 'transparent'}">
            <h3 class="text-capitalize">${arrayOfTasks[index].title}</h3>
            <p class="description text-capitaliz">${arrayOfTasks[index].description}</p>
            <h4 class="category ${arrayOfTasks[index].category} text-capitalize">${arrayOfTasks[index].category}</h4>
...
function changeColor(e) {
    ...
    let tasksInCurrentStatus = arrayOfTasks.filter(task => task.status === status);
    let taskIndex = Array.from(taskElement.parentNode.children).indexOf(taskElement);
    let globalIndex = arrayOfTasks.findIndex(task =>
        task.status === status && task.title === tasksInCurrentStatus[taskIndex].title
    );
    if (globalIndex !== -1) {
